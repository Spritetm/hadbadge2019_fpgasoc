...
#include "libmidi.h"
//#include "ID-Nyan_Cat.h" Todo, add in After Dark Toasters midi
...
extern char _binary_toasters_png_start;
extern char _binary_toasters_png_end;
...
void main(int argc, char **argv) {
	...
	int png_size=(&_binary_toasters_png_end-&_binary_toasters_png_start);
	int i=gfx_load_tiles_mem(GFXTILES, &GFXPAL[0], &_binary_toasters_png_start, png_size);
	...
	while ((MISC_REG(MISC_BTN_REG) & BUTTON_A)==0) {
		...
		int toasters_fr=(cur_vbl_ctr/4)%12;
		...
		for (int y=0; y<3; y++) {
			for (int x=0; x<5; x++) {
				set_sprite(i++, x*16*3+150, y*16*3+100, 16*3, 16*3, toasters_fr*5+x+64*y, 0);
			}
		}

		// Add in music, todo, flying toasters song:
