...
void synth_play_gameboy_monochrome(void){
	...
	synth_queue->voice[0].ctrl     = SYNTH_VOICE_CTRL_ENABLE | SYNTH_VOICE_CTRL_PULSE;
	...
	synth_queue->voice[0].duration = 60;   // ~ quarter note at 125 BPM 
	synth_queue->voice[0].phase_inc = midi_table[87];
	synth_queue->voice_start = (1 << 0);
	synth_queue->cmd_wait = 120 * 256; // factor to line up sample clocks and duration counts
}
