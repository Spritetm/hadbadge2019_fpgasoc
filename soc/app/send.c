...
void getOk(int com, char okval) {
	...
	if (!n) {
		printf("Timeout: No response from dev!\n");
		exit(1);
	}
	char buf=0;
	read(com, &buf, 1);
	if (buf!=okval) {
		printf("Unknown response %x (%c), expected %x (%c)\n", buf, buf, okval, okval);
		exit(1);
	}
	return;
}
...
//	fseek(f, 0x200, SEEK_SET);
	...
	char cmd[3]={'w', (no_bytes>>8)&0xff, no_bytes&0xff};
	write(com, cmd, 3);
	char *p=buff;
	int n=no_bytes;
	int a=0x200;
	while (n) {
		...
		a+=m;
		printf("%04X\n", a);
		for (int i=0; i<m; i++) getOk(com, 'B');
	}
	getOk(com, 'K');
#if 0
	printf("Doing readback of %d bytes...\n", no_bytes);
	char rcmd[3]={'r', (no_bytes>>8)&0xff, no_bytes&0xff};
	write(com, rcmd, 3);
